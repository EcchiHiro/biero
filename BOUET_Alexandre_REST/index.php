...
if($_SERVER['REQUEST_METHOD'] == 'OPTIONS')
{
   ...
   /***************************************************/
	...
	$nomControleur = ucfirst($oReq->url_elements[0]) . "Controleur";		//Ex: BiereControleur
	if(class_exists($nomControleur))	// Si la classe existe
	{
		$oControleur = new $nomControleur;
		$nomAction = strtolower($oReq->verbe) . 'Action';			// Ex. getAction ou putAction
		if(method_exists($oControleur, $nomAction))		// Si la méthode existe
		{	
